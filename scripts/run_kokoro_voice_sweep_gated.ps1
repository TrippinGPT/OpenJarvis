...
$sweepWavCount = Get-Count -Folder $outputRoot -Filter "relay_kokoro_sweep_*.wav"
...
Write-Host ""
Write-Host "Comparison lines" -ForegroundColor Cyan
...
Write-Host "- sweep WAV count: $sweepWavCount"
Write-Host ""
...
$tempPythonScript = Join-Path $env:TEMP ("relay_kokoro_sweep_{0}.py" -f ([guid]::NewGuid().ToString("N")))
$voicesJson = (,@($selectedVoices) | ConvertTo-Json -Compress)
$linesJson = (,@($selectedLines) | ConvertTo-Json -Compress)
...
voices = json.loads("""$voicesJson""")
lines = json.loads("""$linesJson""")
...
try {
    if ($AllowModelDownload -and -not $AllowGenerate) {
        ...
        & $kokoroVenvPython $tempPythonScript "prefetch" $outputRoot $kokoroCachePath $runStamp
    ...
    elseif ($AllowGenerate) {
        ...
        & $kokoroVenvPython $tempPythonScript "generate" $outputRoot $kokoroCachePath $runStamp
        if ($LASTEXITCODE -ne 0) {
